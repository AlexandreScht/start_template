...
export const phoneValidator = z.string().transform((value, ctx) => {
  try {
    const phone = parsePhoneNumber(value, { defaultCountry: 'FR' });
...
export const roleValidator = z.enum(['admin', 'business', 'pro', 'free'], {
  message: 'Invalid choice. The value must be one of the valid options.',
});
